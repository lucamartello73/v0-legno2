...
CREATE TABLE IF NOT EXISTS pergole_configurazioni_tracking (
  ...
  pavimentazione_ids TEXT[], -- Array di ID
  pavimentazione_nomi TEXT[], -- Array di nomi
  pavimentazione_count INTEGER DEFAULT 0,
  
  -- STEP 6: Accessori
  accessori_ids TEXT[], -- Array di ID accessori
  accessori_nomi TEXT[], -- Array di nomi accessori
  ...
  -- Metadata Aggiuntivi (JSON flessibile)
...
-- Policy: Chiunque può inserire (anonimi)
CREATE POLICY "Allow anonymous insert" 
  ON pergole_configurazioni_tracking
  FOR INSERT 
  WITH CHECK (true);

-- Policy: La stessa sessione può aggiornare
CREATE POLICY "Allow session update" 
  ON pergole_configurazioni_tracking
  FOR UPDATE 
  USING (true);

-- Policy: Solo admin possono leggere tutto
...
COMMENT ON COLUMN pergole_configurazioni_tracking.status IS 
  'Stato configurazione: in_progress (in corso), completed (inviata), abandoned (abbandonata)';
